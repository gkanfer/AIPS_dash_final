...
def px_pil_figure(img,bit,mask_name,fig_title,wh):
    ...
    if bit == "bool":
        ...
        # ROI mask
        im_pil = img_as_ubyte(color.gray2rgb(img))
    fig_ch = px.imshow(im_pil, binary_string=True, binary_backend="jpg", width=500, height=500,title=fig_title).update_xaxes(showticklabels=False).update_yaxes(showticklabels = False)
